      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transsition-all ${visible ? "w-full" : "w-0"}`}>
        <div className="flex flex-col text-gray-600">
          <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-3">
            <img src={forward_arrow} alt="dropdown icon" className="h-4 rotate-180" />
            <p>Back</p>
          </div>
          ...
          <NavLink onClick={() => setVisible(false)} className="px-2 pl-6 border" to={`/about-us`}>ABOUT</NavLink>
          <NavLink onClick={() => setVisible(false)} className="px-2 pl-6 border" to={`/conatact-us`}>CONTACT</NavLink>
        </div>
      </div>
